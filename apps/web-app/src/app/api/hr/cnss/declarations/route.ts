...
export async function GET(request: NextRequest) {
  try {
    ...
    const url = `${API_BASE_URL}/api/hr/cnss/declarations${queryString ? `?${queryString}` : ''}`;
...
export async function POST(request: NextRequest) {
  try {
    ...
    const response = await fetch(`${API_BASE_URL}/api/hr/cnss/declarations`, {
      method: 'POST',
      headers: {
