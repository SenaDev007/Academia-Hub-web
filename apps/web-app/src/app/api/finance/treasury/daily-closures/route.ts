...
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.json({ error: 'Failed to fetch daily closures' }, { status: 500 });
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    return NextResponse.json({ error: 'Failed to create daily closure' }, { status: 500 });
  }
}
