...
**Vérification** :
```bash
grep -r "[\u{1F300}-\u{1F9FF}]" apps/next-app/src
```
...
**Vérification** :
```bash
grep -r "from ['\"]lucide-react['\"]" apps/next-app/src
```
...
**Vérification** :
```bash
grep -r "font-montserrat\|font-poppins" apps/next-app/src
```
