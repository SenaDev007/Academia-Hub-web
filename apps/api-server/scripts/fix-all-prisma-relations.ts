/**
 * Script complet pour corriger toutes les relations Prisma manquantes
 */
...
function addRelationIfMissing(modelName: string, relationLine: string) {
...
const fixes = [
  ...
  { model: 'FeeRegimeRule', relation: 'feeRegime FeeRegime @relation("FeeRegimeRuleFeeRegime", fields: [feeRegimeId], references: [id], onDelete: Cascade)' },
  ...
  { model: 'NationalExam', relation: 'examCenters ExamCenter[] @relation("NationalExamCenters")' },
  { model: 'NationalExam', relation: 'examCandidates ExamCandidate[] @relation("NationalExamCandidates")' },
  { model: 'NationalExam', relation: 'nationalExamSubjects NationalExamSubject[] @relation("NationalExamSubjects")' },
  { model: 'NationalExam', relation: 'examResults ExamResult[] @relation("NationalExamResults")' },
  { model: 'NationalExam', relation: 'examDocuments ExamDocument[] @relation("NationalExamDocuments")' },
...
fixes.forEach(({ model, relation }) => {
  if (addRelationIfMissing(model, relation)) {
    added++;
  }
});

console.log(`\n✓ ${added} relations ajoutées`);

// Corriger les relations Patronat
const patronatFixes = [
  { model: 'PatronatUser', field: 'patronatId', relation: 'patronat Patronat? @relation("PatronatUserPatronat", fields: [patronatId], references: [id], onDelete: SetNull)' },
  { model: 'PatronatSchool', field: 'patronatId', relation: 'patronat Patronat? @relation("PatronatSchoolPatronat", fields: [patronatId], references: [id], onDelete: SetNull)' },
  { model: 'NationalExam', field: 'patronatId', relation: 'patronat Patronat? @relation("NationalExamPatronat", fields: [patronatId], references: [id], onDelete: SetNull)' },
  { model: 'QuestionBankResource', field: 'patronatId', relation: 'patronat Patronat? @relation("QuestionBankPatronat", fields: [patronatId], references: [id], onDelete: SetNull)' },
];

patronatFixes.forEach(({ model, field, relation }) => {
  const modelRegex = new RegExp(`(model ${model}\\s*\\{[^}]*?)(\\s*tenantId)`, 's');
  if (schema.match(modelRegex) && !schema.includes(`${model}.*${field}`)) {
    // Vérifier si le champ existe déjà
    const fieldRegex = new RegExp(`${field}\\s+String`, 's');
    if (fieldRegex.test(schema)) {
      // Le champ existe, ajouter la relation
      const relationRegex = new RegExp(`(model ${model}[^}]*?)(\\s*@@map|\\s*\\})`, 's');
      const match = schema.match(relationRegex);
      if (match && !match[1].includes(relation.split(' ')[0])) {
        schema = schema.replace(relationRegex, `$1\n  ${relation}\n$2`);
        console.log(`✓ Ajouté relation patronat dans ${model}`);
      }
    }
  }
});
