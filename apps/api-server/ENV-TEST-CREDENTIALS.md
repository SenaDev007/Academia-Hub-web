# 🔐 Configuration des Identifiants de Test - API Server

Ce document décrit les variables d'environnement nécessaires pour les identifiants de test dans l'API server.
...
Créez un fichier `.env` dans `apps/api-server/` avec les variables suivantes :
...
# ============================================================================
TEST_SUPER_ADMIN_EMAIL=yehiortech@gmail.com
TEST_SUPER_ADMIN_PASSWORD=C@ptain.Superadmin1
...
- Les variables d'environnement sont utilisées par le script de seed (`seed-tenant-cspeb.ts`)
- Le fichier `.env` n'est pas versionné (déjà dans `.gitignore`)
- En production, configurez ces variables dans votre plateforme de déploiement
